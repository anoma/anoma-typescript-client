import { AnomaClient } from "./index";
import { Intent } from "./protobufs/intents/intent_pb";

function main() {
  let client = new AnomaClient("localhost:50052", "some_node");

  // An arbitrary Intent
  // (created in Elixir using Examples.ETransparent.ETransaction.single_swap |> Noun.Nounable.to_noun |> Nock.Jam.jam |> Base.encode64)
  const intent = new Intent();
  intent.setIntent(
    ""
  );

  // List all intents
  client.listIntents(function (err, response) {
    console.log("list intents");
    console.log(response);
    console.log(err);
  });

  // Add an intent
  client.addIntent(intent, function (err) {
    console.log("add intent");
    console.log(err);
  });

  // Compose two intents
  client.composeIntents(intent, intent, function (err, response) {
  ...
  // Verify an intent
  client.verifyIntent(intent, function (err, result) {
    console.log("verify intent");
    console.log(result);
    console.log(err);
  });

  // List nullifiers
  client.listNullifiers(function (err, result: any) {
    console.log("nullifiers");
    console.log(result);
    console.log(err);
  });

  // List unrevealed commits
  client.listUnrevealedCommits(function (err, result: any) {
    console.log("unrevealed commits");
    console.log(result);
    console.log(err);
  });

  // List unspent resources
  client.listUnspentResources(function (err, result: any) {
    console.log("unspent resources");
    console.log(result);
    console.log(err);
  });

  // Get block
  client.getBlockBefore(1234, function (err, result) {
    console.log("get block before");
    console.log(result);
    console.log(err);
  });

  // Get block
  client.getBlockAfter(1234, function (err, result) {
    console.log("get block before");
    console.log(result);
    console.log(err);
  });
  ...
  // Get root
  client.getRoot(function (err, result) {
  ...
  // Get root
  client.getLatest(function (err, result) {
  ...
  // Get root
  client.filter(["some kind"], ["some owner"], function (err, result) {
    console.log("get latest block");
    console.log(result);
    console.log(err);
  });

  client.addToMempool("some tx", function (err) {
    console.log("add to mempool");
    console.log(err);
  });

  client.dumpMempool(function (err, transaction_candidates) {
    console.log("dump mempool");
    console.log(transaction_candidates);
    console.log(err);
  });
  ...
  const inputs: string[] = ["aA=="];

  client.prove(juvix_squared, inputs, [], function (err, result) {
    console.log("prove nock");
    console.log(result);
    console.log(err);
  });

  client.run(juvix_squared, [], function (err, result) {
    console.log("run nock");
    console.log(result);
    console.log(err);
  });
}
